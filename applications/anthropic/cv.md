# Yann Burden

**AI and Technology Executive. Exited Founder. Enterprise AI Go-to-Market.**

+61 400 941 979 | yannburden@gmail.com | energyreturn.co/me | linkedin.com/in/yannburden

Working rights: Australia, UK, EU, Canada

---

## Highlights

- Three enterprise AI companies built from the ground up, all acquired. Consistently the technical voice in the room for enterprise sales, scoping, POC execution, and customer success.
- Founded Billcap, a vertical ML platform for energy retailers. Ran the enterprise sales and technical engagement for 5 clients across 250,000+ end users. Sold to Tally Group for 8x investor return.
- Hired as CPTO at Pendula, shipped agentic customer communication workflows in production -- intent classification, next-best-action recommendation, conversational AI agents with LLMs and RAG. Led the technical pre-sales conversations that unlocked $5M incremental ARR.
- Established responsible AI practices for production customer-facing systems: model monitoring, human-in-the-loop review, bias detection, and governance documentation.
- Opened Pendula's London office post Series A, signed first UK client -- demonstrating ability to scale AI products and enterprise relationships across markets.
- Venture Partner at Black Nova VC, advising early-stage portfolio companies on AI strategy, product development, and enterprise go-to-market.

## Focus

Enterprise AI Adoption | Technical Pre-Sales Leadership | Agentic Workflows | AI Governance | ANZ Enterprise Customers | Cross-Market Scaling | Regulated Industries

---

## Experience

### Pendula | 2021 - 2024
**Chief Product and Technology Officer / Chief AI Officer**

- Led 25-person engineering, product and data team through to acquisition by Smart Communications.
- Shipped agentic customer communication workflows in production: intent classification for inbound message routing, next-best-action recommendation engines for outbound orchestration, and sentiment analysis pipelines processing millions of customer interactions.
- Built and deployed conversational AI agents using LLMs for enterprise customer communication -- prompt engineering, retrieval-augmented generation, evaluation frameworks, and production monitoring.
- Led technical pre-sales for enterprise accounts: scoped AI use cases, built technical champions within customer organisations, executed POCs, and validated ROI -- directly contributing to $5M incremental ARR.
- Established responsible AI practices for customer-facing systems: model monitoring, bias detection, human-in-the-loop review for high-stakes communications, and documentation standards for AI governance.
- Architected data pipeline infrastructure: event streaming, feature stores, and model serving on AWS.
- Set up London offices post Series A, signed first UK deal.

### DC Power Co | 2018 - 2021
**Chief Customer and Technology Officer**

- Founding executive team of Australia's first crowd-funded renewable energy retailer. Customer book acquired by Ion Group.
- Developed in-house customer engagement product reducing churn by over 25%, using predictive churn models built on usage patterns, billing data, and behavioural signals.
- Delivered strategic NPS of +51 in first year of operation.
- Set up digital sales channel delivering market-leading conversion rates for battery storage.

### Billcap | 2011 - 2018
**CEO and Co-founder**

- Co-founded and led a vertical customer engagement platform for energy retailers, driving retention, cross-sell, and personalisation through ML-powered insights.
- Ran the full enterprise sales cycle: prospecting, technical scoping, POC execution, contract negotiation, and ongoing customer success for 5 enterprise clients across 250,000+ end users.
- Prototyped ML models in R for outlier detection, consumption prediction, and household segmentation, working directly with smart meter interval data.
- Established research partnership with the University of Melbourne -- platform data underpinned three peer-reviewed publications in top economics journals.
- Led sale of company to Tally Group for 8x investor return.

---

## Earlier Experience

- **Energy Return** | 2009 - 2011 | Founder and MD. Energy efficiency consulting for local government.
- **Cool Nrg** | 2007 - 2009 | General Manager (France). Nation-wide residential energy efficiency and carbon abatement for energy retailers.
- **Accenture (UK)** | 2005 - 2007 | Technology Strategy Manager. CIO advisory, IT organisation design, telco strategy.
- **Unilog (France)** | Technology Strategy. Energy retail focus.
- **PwC Consulting** | SAP Consultant.

---

## Advisory and Boards

**Black Nova Venture Capital** -- Venture Partner, AI Advisory (2025 -)

**University of Melbourne** -- Honorary Industry Fellow, School of Computing and Information Systems (2015 - 2024)

**Bendigo Bank Community Bank** -- Non-Executive Director (2010 - 2017)

---

## Education

**University of Melbourne** -- Bachelor of Science (Information Systems), 1998

---

## Press and Media

- **[SmartCompany](https://www.smartcompany.com.au/startupsmart/eight-aussie-startups-raised-149-million-week/)** (Aug 2023): Featured in coverage of Pendula's $14.5 million raise from MA Growth Ventures and Octopus Ventures.
- **[Tech Trajectory Podcast, DiUS](https://techtrajectorypodcast.buzzsprout.com/2457356/episodes/17422143-leading-with-customer-clarity-in-a-changing-world)** (Jul 2025): In conversation with Kavita Karwar on AI adoption, product strategy, and customer clarity.

## Research Collaborations

Billcap's smart meter data underpinned a multi-year research programme with the University of Melbourne.

- **Tell Me Something I Don't Already Know.** Byrne, La Nauze, Martin. *Review of Economics and Statistics*, 100(3), 2018.
- **An Experimental Study of Monthly Electricity Demand (In)elasticity.** Byrne, La Nauze, Martin. *The Energy Journal*, 42(2), 2021.
- **Power from the People.** La Nauze. *Journal of Political Economy*, 127(3), 2019. Acknowledgements: "I thank Yann Burden and Billcap for access to proprietary data."

---

## Technical

- **Hands-on with current AI tooling.** Day-to-day agentic development in Claude Code and Cursor -- spec-driven, TDD-aware. Python, TypeScript, SQL. My current side project (a solar panel performance assessment app) is built end-to-end on this loop.
- **I design and ship agent systems.** Multi-agent orchestration over enterprise data, with cost and quality instrumented from day one. CrewAI, Anthropic SDK, AWS Bedrock, Langfuse. Led Pendula's AI-powered analytics layer -- so non-engineers could query data directly instead of waiting on the data team.
- **Production AI, not demos.** Intent classification, next-best-action scoring, conversational agents with RAG and tool use. The substrate behind Pendula's $5M ARR uplift.
- **Cloud, data, ops.** AWS, GCP, Postgres, event streaming, MLOps. Senior enough to architect, hands-on enough to spot issues myself.
