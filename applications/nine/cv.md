# Yann Burden

**AI Product Leader. Exited Founder.**

+61 400 941 979 | yannburden@gmail.com | energyreturn.co/me | linkedin.com/in/yannburden

Working rights: Australia, UK, EU, Canada

---

## Highlights

- Led product and engineering at Pendula, defining AI product strategy and owning the end-to-end product lifecycle for an ML-driven customer engagement platform spanning SMS, email, push, and voice channels. Navigated the product through acquisition by Smart Communications.
- Founded and scaled Billcap, a vertical ML product for energy retailers. Built the product from concept through to exit, managing the full product lifecycle including roadmap, stakeholder alignment, and commercial outcomes.
- Track record of translating complex AI capabilities into tangible product experiences that drive measurable business outcomes -- $5M incremental ARR at Pendula, 25% churn reduction at DC Power Co.
- Experienced in multi-channel product strategy, coordinating AI-driven personalisation and decisioning across diverse platforms and audience segments.
- Strong executive stakeholder management -- reported to CEO and board at both Pendula and DC Power Co, aligning product vision with business strategy across C-suite, investors, and enterprise clients.
- Venture Partner at Black Nova VC, advising early-stage portfolio companies on AI strategy, product development, and go-to-market.

## Focus

AI Product Management | Multi-Channel Product Strategy | Stakeholder Management | Customer Engagement | Personalisation | Scale-Up SaaS

---

## Experience

### Pendula | 2021 - 2024
**Chief Product and Technology Officer**

- Led product strategy and a 20-person engineering, product and data organisation through to acquisition by Smart Communications.
- Defined and delivered the AI product roadmap: intent classification for inbound message routing, next-best-action recommendations for outbound orchestration, and conversational AI agents -- shipped across SMS, email, push, and voice channels.
- Managed the product lifecycle end-to-end, from discovery and prioritisation through delivery and iteration, working cross-functionally with data scientists, engineers, designers, and business stakeholders.
- Drove AI adoption across the organisation, championing ML-driven personalisation as the platform's core differentiator and leading internal education on AI capabilities and limitations.
- Unlocked additional $5M ARR by reorienting the product toward enterprise client needs, with AI-driven personalisation as the key value driver.
- Engaged senior executive stakeholders including CEO, board, and enterprise client leadership to align product direction with business objectives and commercial strategy.
- Implemented responsible AI practices for customer-facing products: model monitoring, evaluation frameworks, and governance processes.
- Set up London offices post Series A, adapting the product for international market entry and signing the first UK client.

### DC Power Co | 2018 - 2021
**Chief Customer and Technology Officer**

- Founding executive team of Australia's first crowd-funded renewable energy retailer.
- Developed in-house customer engagement product reducing churn by over 25%, using predictive churn models built on usage patterns, billing data, and behavioural signals.
- Delivered strategic NPS of +51 in first year of operation through product-led customer experience design.
- Set up digital sales channel delivering market-leading conversion rates for battery storage.

### Billcap | 2011 - 2018
**CEO and Co-founder**

- Co-founded and led a vertical ML product for energy retailers, owning product vision, roadmap, and delivery from inception through to exit.
- Built and iterated the product across 5 energy company clients, each with distinct user needs, regulatory requirements, and data environments -- managing prioritisation and stakeholder alignment across all accounts.
- Prototyped ML models in R for consumption prediction, household segmentation, and anomaly detection, working directly with smart meter interval data across 250,000+ users.
- Established research partnership with the University of Melbourne, resulting in publications in the Journal of Political Economy and the Review of Economics and Statistics.
- Led sale of company to Tally Group.

---

## Earlier Experience

- **Energy Return** | 2009 - 2011 | Founder and MD. Energy efficiency consulting for local government.
- **Cool Nrg** | 2007 - 2009 | General Manager (France). Nation-wide residential energy efficiency and carbon abatement for energy retailers.
- **Accenture (UK)** | 2005 - 2007 | Technology Strategy Manager. CIO advisory, IT organisation design, telco strategy.
- **Unilog (France)** | Technology Strategy. Energy retail focus.
- **PwC Consulting** | SAP Consultant.

---

## Advisory and Boards

**Black Nova Venture Capital** -- Venture Partner, AI Advisory (2025 -)

**University of Melbourne** -- Honorary Industry Fellow, School of Computing and Information Systems (2015 - 2024)

**Bendigo Bank Community Bank** -- Non-Executive Director (2010 - 2017)

---

## Education

**University of Melbourne** -- Bachelor of Science (Information Systems), 1998

---

## Press and Media

- **SmartCompany** (Aug 2023): Named and pictured as Chief Growth Officer in coverage of Pendula's $14.5 million raise from MA Growth Ventures and Octopus Ventures. [smartcompany.com.au](https://www.smartcompany.com.au/startupsmart/eight-aussie-startups-raised-149-million-week/)
- **Tech Trajectory Podcast, DiUS** (Jul 2025): In conversation with Kavita Karwar on AI adoption, product strategy, and customer clarity. [Buzzsprout](https://techtrajectorypodcast.buzzsprout.com/2457356/episodes/17422143-leading-with-customer-clarity-in-a-changing-world)

## Research Collaborations

Billcap's smart meter data underpinned a multi-year research programme with the University of Melbourne.

- **Tell Me Something I Don't Already Know: Informedness and the Impact of Information Programs.** Byrne, La Nauze, Martin. *Review of Economics and Statistics*, 100(3), 2018. [doi.org/10.1162/rest_a_00695](https://doi.org/10.1162/rest_a_00695)
- **An Experimental Study of Monthly Electricity Demand (In)elasticity.** Byrne, La Nauze, Martin. *The Energy Journal*, 42(2), 2021. [doi.org/10.5547/01956574.42.2.dbyr](https://doi.org/10.5547/01956574.42.2.dbyr)
- **Power from the People: Rooftop Solar and a Downward-Sloping Supply of Electricity.** La Nauze. *Journal of Political Economy*, 127(3), 2019. [doi.org/10.1086/705535](https://doi.org/10.1086/705535)

---

## Technical

Python | SQL | R | AWS | GCP | Scikit-learn | TensorFlow | Pandas | Spark | PostgreSQL | Event streaming | MLOps
